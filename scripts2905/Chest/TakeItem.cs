...
public class TakeItem : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "pItem")
        {
            switch (collision.gameObject.name)
            {
                case "speedBoots_sprite":
                    TakeItemSpeedBoots(collision.gameObject);
                    break;
                case "explosions_sprite":
                    collision.gameObject.GetComponent<ExplosionsSprite>().OnTaking();
                    break;
            }
        }
    }
    ...
    void TakeItemSpeedBoots(GameObject item) {
        FakeDestroying(item);
        item.GetComponent<SpeedBootsSprite>().OnTaking();
    }

...
public class Timer : MonoBehaviour
{
    ...
    bool SpawnEnemy(GameObject room, GameObject enemy) {
        int randCount = room.gameObject.GetComponent<TechSpawner>().EnemySpawners.Length;
        randCount = Random.Range(0, randCount);
        GameObject spawner = room.gameObject.GetComponent<TechSpawner>().EnemySpawners[randCount];
        if (spawner.GetComponentInChildren<EnemySpawn>().isOnCollider == true)
        {
            return false;
        }
    ...
    void FullSpawnEnemy(int difficult = 1) {
        ...
        while (CheckAllRooms != 1000)
        {
            ...
            {
                if (SpawnEnemy(newRoom, newEnemy) == true) {
                    break;
                }
            }
        }
